import {
  DELETED_SUCCESS,
  MESSAGE_API_SEND_MSG_SUCCESS,
  SOMETHING_WRONG,
  STORE_SUCCESS,
} from "App/Helpers/Languange";
...
class OutboxService {
  public async lists(userUuid: string, authent: string) {
    ...
    if (authent === "superadmin") {
      //sender
      const model = await Outbox.query()
      ...
      model.forEach((element) => {
        const row = {};
        row["id"] = element.uuid;
        row["tanggal"] = DateTime.fromISO(element.createdAt).toFormat(
          "dd/MM/yyyy H:MM:ss"
        );
        ...
        row["status"] =
          element.status == "waiting"
            ? { color: "grey", text: "Menunggu Antrian" }
            : element.status == "waiting"
            ? { color: "orange", text: "Proses Pengiriman" }
            : { color: "green", text: "Terkirim" };
  ...
  public async store(payload: OutboxInterface) {
    try {
      ...
      const data = {
        ...
        recieveNumber: await Utility.phoneNumberFormatter(
          payload.recieveNumber
        ),
        type: payload.type,
  ...
  public async storeFromApiService(payload: OutboxServiceApiType) {
    try {
      ...
      await Queue.dispatch("App/Jobs/SendMessage", dataOfJob, {
        removeOnComplete: 10,
        removeOnFail: 10,
        repeat: { every: 6000 },
        jobId: jobId,
  ...
  public async bulkStoreFromApiService(
    payload,
    senderNumber: string,
    userUuid: string,
    userName: string
  ...
  public async bulkStoreFromApiServiceSendBox(
    payload,
    senderNumber: string,
    userUuid: string,
    userName: string
